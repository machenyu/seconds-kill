...
![](<https://github.com/machenyu/seconds-kill/blob/master/assets/Jmeter.jpeg>)
...
超卖问题可以通过悲观锁解决，但是加锁的时间可能会很长，会长时间的限制其他用户的访问，导致很多请求等待锁，卡死在这里，如果这种请求很多就会耗尽连接，系统出现异常。乐观锁默认不加锁，失败就直接返回抢购失败，可以承受较高并发

![](<https://github.com/machenyu/seconds-kill/blob/master/assets/%E4%B9%90%E8%A7%82%E9%94%81%E6%89%A3%E5%BA%93%E5%AD%98.png>)
...
由于计数限流实现起来比较简单，因此采用计数限流，限流的实现可以直接使用 Guava 的 RateLimit 方法，这里选用 Redis 实现分布式限流
...
![](<https://github.com/machenyu/seconds-kill/blob/master/assets/%E7%A7%92%E6%9D%80%E9%99%90%E6%B5%81.png>)
...
虽然限流能够过滤掉一些无效的请求，但是还是会有很多请求落在数据库上，对于每个没有被过滤掉的请求，都会去数据库查询库存来判断库存是否充足，对于这个查询可以放在缓存 Redis 中，Redis 的数据是存放在内存中的，速度快很多。

![](<https://github.com/machenyu/seconds-kill/blob/master/assets/Redis%20%E7%BC%93%E5%AD%98%E5%BA%93%E5%AD%98%E4%BF%A1%E6%81%AF.png>)
...
缓存和 DB 的一致性是一个讨论很多的问题，缓存不做更新，仅做删除，先更新数据库再删除缓存。A 更新了数据库，还没来得及删除缓存，B 又更新了数据库，接着删除了缓存，然后 A 删除了缓存，这样只有下次缓存未命中时，才会从数据库中重建缓存，避免了脏数据。但是，也会有极端情况出现脏数据，A 做查询操作，没有命中缓存，从数据库中查询，但是还没来得及更新缓存，B 就更新了数据库，接着删除了缓存，然后 A 又重建了缓存，这时 A 中的就是脏数据，如下图所示。但是这种极端情况需要数据库的写操作前进入数据库，又晚于写操作删除缓存来更新缓存，发生的概率极其小，不过为了避免这种情况，可以为缓存设置过期时间。

![](<https://github.com/machenyu/seconds-kill/blob/master/assets/%E5%85%88%E6%9B%B4%E6%96%B0%E6%95%B0%E6%8D%AE%E5%BA%93%E5%86%8D%E5%88%A0%E9%99%A4%E7%BC%93%E5%AD%98.png>)
...
![](<https://github.com/machenyu/seconds-kill/blob/master/assets/%E5%85%88%E6%9B%B4%E6%96%B0%E6%95%B0%E6%8D%AE%E5%BA%93%E5%86%8D%E6%9B%B4%E6%96%B0%E7%BC%93%E5%AD%98V2.png>)
...
![](<https://github.com/machenyu/seconds-kill/blob/master/assets/%E6%B6%88%E6%81%AF%E9%98%9F%E5%88%97%E7%BC%93%E5%86%B2.png>)

向 Kafka 发送消息和从 Kafka 拉取消息需要对消息进行序列化处理，这里采用的是`Gson`框架
...
>- [极客时间：唐扬 - 高并发系统设计40问](<https://time.geekbang.org/column/intro/100035801>)
>- [极客时间：李玥 - 消息队列高手课](<https://time.geekbang.org/column/intro/100032301>)
